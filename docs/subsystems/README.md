...
| [browser.md](browser.md) | the browser capability seam this fork adds: the `ctx.browser` service (provider registry, tab operations, humanized input) with its provider contract and `browser/provider-updated` event |
