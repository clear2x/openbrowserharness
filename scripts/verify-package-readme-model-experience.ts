...
const SENTENCE_MODEL_EXPERIENCE: Readonly<Record<string, SentenceContract>> = {
  ...
  'packages/fs/fs-opfs': { kind: 'indirect', reason: 'The OPFS provider backend delegates model rendering to dsh-tool-fs.' },
  ...
  'packages/host/apiproxy': { kind: 'none', reason: 'The wire contract and carriers forward already-assembled provider calls and streamed responses; nothing here renders, gates, or stores model content.' },
  ...
  'packages/session/session-persistence-indexeddb': { kind: 'none', reason: 'The IndexedDB persistence backend stores durable session state and registers nothing model-facing.' },
