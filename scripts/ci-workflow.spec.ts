...
// path (.github/workflows.upstream/); these assertions describe upstream's
// CI design against that archive — the fork's live workflows diverge on purpose.
describe('Archived upstream CI workflows', () => {
  it.each(['ci.yml', 'ci-master.yml', 'e2e.yml', 'release.yml', 'release-vendor.yml'])(
    '%s cancels superseded validation runs without crossing workflow or ref boundaries', (name) => {
      const workflow = loadWorkflow('.github/workflows.upstream/' + name)
  ...
  it('does not cancel protected publication or deployment transactions', () => {
    for (const name of ['release-publish.yml', 'release-vendor-publish.yml']) {
      const publish = workflowJob(loadWorkflow('.github/workflows.upstream/' + name), 'publish')
    ...
    for (const name of ['python-release.yml', 'node-addon-system-release.yml', 'docs-pages.yml']) {
      expect(loadWorkflow('.github/workflows.upstream/' + name).concurrency).toMatchObject({ 'cancel-in-progress': false })
    }
  })

  it('skips coverage-history uploads on cancellation but retains Wine cleanup', () => {
    const coverage = workflowJob(loadWorkflow('.github/workflows.upstream/ci.yml'), 'windows-coverage')
    const wine = workflowJob(loadWorkflow('.github/workflows.upstream/ci-master.yml'), 'windows')
  ...
  it('isolates every pnpm action setup destination per runner', () => {
    const files = ['.github/workflows.upstream/ci.yml', '.github/workflows.upstream/ci-master.yml']
  ...
  it.each(['node-24', 'node-24-coverage', 'node-24-consumers'])(
    ...
    (jobName) => {
      const job = workflowJob(loadWorkflow('.github/workflows.upstream/ci.yml'), jobName)
  ...
  it('isolates the python SDK exe pnpm setup destination per job', () => {
    const workflow: unknown = yaml.load(readFileSync(resolve(root, '.github/workflows.upstream/build-exe-for-python-sdk.yml'), 'utf8'))
  ...
  it('keeps split native Windows PR jobs with failover, plus a master-only standby', () => {
    ...
    const masterWorkflow = loadWorkflow('.github/workflows.upstream/ci-master.yml')
  ...
  it('gates standalone keyless blacksmith jobs and benchmark tiers on the failover variables', () => {
    const expectedFilenames = workflowJob(loadWorkflow('.github/workflows.upstream/expected-filenames.yml'), 'expected-filenames')
    const sandbox = workflowJob(loadWorkflow('.github/workflows.upstream/sandbox.yml'), 'sandbox-e2e')
    ...
    for (const name of ['larger-runner-benchmark', 'consolidated-runner-benchmark'] as const) {
      const benchmark = workflowJob(loadWorkflow('.github/workflows.upstream/ci-master.yml'), name)
  ...
  it('always restores the hosted benchmark pnpm cache', () => {
    const benchmark = workflowJob(loadWorkflow('.github/workflows.upstream/ci.yml'), 'node-24-bench')
  ...
  it('cancels superseded master runs without changing the post-merge job inventory', () => {
    const workflow = loadWorkflow('.github/workflows.upstream/ci-master.yml')
    const prWorkflow = loadWorkflow('.github/workflows.upstream/ci.yml')
...
describe('DeepSeek e2e workflow', () => {
  it('prepares bubblewrap from the pinned payload without a package transaction', () => {
    const workflow = loadWorkflow('.github/workflows.upstream/e2e.yml')
...
describe('Weighted approval workflow', () => {
  it('publishes from the trusted default branch after pull request and review updates', () => {
    const publisher = loadWorkflow('.github/workflows.upstream/weighted-approval.yml')
    const reviewEvent = loadWorkflow('.github/workflows.upstream/weighted-approval-review-event.yml')
...
describe('npm release workflows', () => {
  it('keeps publication dispatch-only and pack in the PR workflow', () => {
    ...
    for (const file of ['release.yml', 'release-vendor.yml']) {
      const workflow = loadWorkflow(`.github/workflows.upstream/${file}`)
  ...
  it('runs dependency policy and npm layout checks in the DSH release workflow', () => {
    const workflow = loadWorkflow('.github/workflows.upstream/release.yml')
...
describe('Documentation site publication', () => {
  it('keeps Pages deployment dispatch-only from a dsh-v* tag', () => {
    const workflow = loadWorkflow('.github/workflows.upstream/docs-pages.yml')
    const build = workflowJob(workflow, 'build')
