...
function makeProvider(id: string, overrides: Partial<BrowserProvider> = {}): BrowserProvider {
  ...
  return {
    ...
    duplicateTab: () => Promise.resolve({ ...tab, tabId: 3 }),
    updateTabPinned: () => Promise.resolve(),
    updateTabMuted: () => Promise.resolve(),
    moveTab: () => Promise.resolve(),
    closeOtherTabs: () => Promise.resolve(0),
    reopenClosedTab: () => Promise.resolve(undefined),
    listWindows: () => Promise.resolve([{ windowId: 1, focused: true, tabCount: 1 }]),
