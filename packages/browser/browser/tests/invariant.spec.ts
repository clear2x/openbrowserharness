  ...
  it('accepts real provider-set changes', async () => {
    ...
    try {
      ...
      const disposeProvider = ctx.browser.register({
        ...
        closeTab: noop,
        reloadTab: noop,
        duplicateTab: () => Promise.resolve({ tabId: 1, title: '', url: '', active: true, windowId: 1, index: 0 }),
        updateTabPinned: noop,
        updateTabMuted: noop,
        moveTab: noop,
        closeOtherTabs: () => Promise.resolve(0),
        reopenClosedTab: () => Promise.resolve(undefined),
        listWindows: () => Promise.resolve([{ windowId: 1, focused: true, tabCount: 0 }]),
        focusWindow: noop,
        navigate: noop,
