...
function guidance(blockedAfter: number): string {
  return 'Use goal tools for one long-running completion objective in the current session. '
    ...
    + 'resume to rearm it. A paused or blocked goal stays parked: never act on it on your own — '
    + 'a new human task always takes priority, and only an explicit human resume of that goal '
    + 'justifies update_goal action resume. Mark complete only when the objective is actually '
    + `achieved. Mark blocked only after the same blocking condition persists for at least ${blockedAfter} `
