...
没有直接可见内容：本后端不注册 prompt 或 schema，也从不改写会话内容——协调器的合成收尾事件（`step/end`、`turn/end {interrupted}`、可重试的工具错误）是仅有的日志变化，与 JSONL/SQLite 后端一致。

#### Token 影响

零直接 token 影响；经本后端的续接/历史读取只馈送到所组合 agent 已有的渲染。

#### KV Cache 影响

没有直接失效；持久化从不改动已组装的请求前缀。
