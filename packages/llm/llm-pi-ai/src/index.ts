...
export function apply(ctx: Context, config: Config): void {
  ...
  // The request headers a named route's profile carries, asked alongside the
  // stored credential so an interrogation of an already-configured route
  // travels the way a real request through it will. Resolved profiles keep the
  // headers record detached from configuration, so handing it over shares no
  // mutable state with requests.
  const storedHeaders = async (provider: string | undefined): Promise<Readonly<Record<string, string>> | undefined> => {
    if (provider === undefined) return undefined
    return profiles().get(provider)?.headers
  }
  ...
  // except the credential and headers: a configuration surface edits a
  // redacted descriptor and never holds a stored secret, so an
  // already-configured route supplies both here rather than being interrogated
  // unauthenticated.
  ctx.llm.registerModelDiscovery(
    NS,
    request => discoverModels(request, () => storedApiKey(request.provider), () => storedHeaders(request.provider)),
  )
