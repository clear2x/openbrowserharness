/**
 * Pure helpers over one provider profile's model catalog: capacity spelling,
 * draft coercion, and adapter-constraint validation. Shared by the route
 * editor and the create wizard; the interactive row editors render around
 * these contracts.
 * @module @deepseek-ai/dsh-client-ui-settings-models/modelCatalog
 */
 ...
 * @param value - a `models` array as a form holds it.
 * @returns the first invalid row, or undefined when the adapter will accept it.
 */
export function validateModelCatalog(value: unknown): ModelCatalogFailure | undefined {
