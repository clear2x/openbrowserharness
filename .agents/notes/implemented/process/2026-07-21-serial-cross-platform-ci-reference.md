...
The workflow contains duplicated setup steps and a main reference run can take much longer than the optimized pull-request path. That duplication is deliberate: reviewers can inspect each operating system's complete command without resolving a matrix or concurrent scheduler.
...
A sandbox regression visible only to a real host kernel or the packed Landlock install can merge before the main run reports it. That post-merge detection window is accepted in exchange for removing four non-blocking jobs from every pull request; the default branch retains the complete signal.
