...
- Boot mirrors the web client's pattern (`packages/client/web/src/boot.ts`): `new Context()` → `ctx.plugin(Loader)` → a **static module map** (package name → namespace import) replaces `app-boot`'s yml-file composition. No `cordis.yml`, no `!!js`.
